...
def assembler_parse_line(line: str) -> str:
    if DEBUG_PRINT:
    ...
    opcode_str = f"{opcode:b}".zfill(7)
    binary_strings.appendleft(opcode_str)

    def process_reg():
        reg_name = tokens.popleft().lower()
        reg_num = REGISTER_LOOKUP[reg_name]
        binary_strings.appendleft(f"{reg_num:b}".zfill(6))

    def process_imm(jal=False):
        imm = int(tokens.popleft())
...
if __name__ == "__main__":
    ...
    with open(src_filename, "r") as f:
        lines = f.readlines()
        for line in lines:
            # get rid of newlines and hashtag comments
            line = line.strip()
            index = line.find("#")
            if index != -1:
                line = line[0:index]
            ...
            if len(line) > 0:
                machine_codes.append(assembler_parse_line(line))
